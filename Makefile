LANGUAGE_NAME := tree-sitter-devnotes
HOMEPAGE_URL := https://github.com/example/devnotes
VERSION := 0.1.0
DESCRIPTION := tree-sitter grammar for the DevNotes meta-language
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/devnotes '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/devnotes
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/devnotes
